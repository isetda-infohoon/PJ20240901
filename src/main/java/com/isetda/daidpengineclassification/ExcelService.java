...
//                            }
    ...
    public void jsonDataWithUpdate(String fileName, String[] values) throws UnirestException {
        ...
        for (String ext : extensions) {
            fileInfo = ClassificationService.getFileByName(userId, name + ext);
...
//            }
            ...
            if (processedCount == totalPageCount) {
                FileInfo resultFileInfo = ClassificationService.getFileByNameAndStatus(userId, basename + "-page1.jpg", "CS");
                if (resultFileInfo == null || resultFileInfo.getFilename() == null) {
                    resultFileInfo = ClassificationService.getFileByNameAndStatus(userId, basename + "-page1.jpg", "CF");
                }
                ...
                if (resultFileInfo != null && resultFileInfo.getFilename() != null) {
                    ...
                    ClassificationService.callUpdateApi(pdfJsonBody);
                } else {
...
//        apiCaller.callUpdateApi(jsonBody);
    ...
    public void jsonDataUpdateWithUnitFile(String taskName, String fileName, String[] values) throws UnirestException {
        ...
        for (String ext : FileExtensionUtil.DA_SUPPORTED_EXT) {
            fileInfo = ClassificationService.getFileByName(userId, name2 + "." + ext);
            if (fileInfo != null && fileInfo.getFilename() != null) {
                officeExt = ext;
...
//                log.info("classificationEndDateTime : " + resultFileInfo.getClassificationEndDateTime());
                ...
                FileInfo checkFileName = ClassificationService.getFileByName(configLoader.apiUserId, basename + ".pdf");
                ...
                FileInfo originalFileInfo = ClassificationService.getFileByNameAndPageNumAndStatusNotNull(userId, basename + ".pdf", "CS", 0);
                if (originalFileInfo.getFilename() == null) {
                    originalFileInfo = ClassificationService.getFileByNameAndPageNumAndStatusNotNull(userId, basename + ".pdf", "CF", 0);
                }

                if (configLoader.useCallbackUpdate && configLoader.serviceType.contains("da")) {
                    ClassificationService.callbackApi(originalFileInfo, pdfResultPath.getPath(), 200, "완료");
                }
            }
...
//            }
        ...
        if (fileInfo.getPageNum() == 0 && imageExt != null && !imageExt.equals(".pdf")) {
            ...
            ClassificationService.callUpdateApi(jsonBody);
            if (configLoader.useCallbackUpdate && configLoader.serviceType.contains("da")) {
                ClassificationService.callbackApi(fileInfo, imgResultPath.getPath(), 200, "완료");
            }
        }
        ...
        if (fileInfo.getPageNum() == 0 && officeExt != null && !officeExt.equals(".pdf")) {
            ...
            ClassificationService.callUpdateApi(jsonBody);
            if (configLoader.useCallbackUpdate && configLoader.serviceType.contains("da")) {
                ClassificationService.callbackApi(fileInfo, officeResultPath.getPath(), 200, "완료");
            }
        }
    }
    ...
    public int getProcessedPageCount(String userId, String basename) throws UnirestException {
        ...
        List<FileInfo> filesWithCS = ClassificationService.getAllFilesWithStatus(userId, "CS");
        ...
        List<FileInfo> filesWithCF = ClassificationService.getAllFilesWithStatus(userId, "CF");
                        ...
                        if (value != null) {
                            ...
                            if (!configLoader.apiUsageFlag) {
                                ...
                                try {
                                    ...
                                    // 현재 파일의 원본 파일 정보
                                    currentFileInfo = ClassificationService.getFileByName(configLoader.apiUserId, subPath + origName + origExt);
                                ...
                                // 현재 파일의 이미지 파일 정보
                                FileInfo fileInfo = ClassificationService.getFileByName(configLoader.apiUserId, subPath + baseName + ".jpg");
...
//                            : resultByVersion.get(baseName);
            ...
            for (String ext : FileExtensionUtil.DA_SUPPORTED_EXT) {
                try {
                    fileInfo = ClassificationService.getFileByName(configLoader.apiUserId, subPath + originalName + "." + ext);
